...
⚠️ [주의 — 현재 서버 구조에서는 1.0 일치가 나오지 않음]
  embed_server.py가 Qwen3 공식 비대칭 방식으로 변경되면서 /embed는 모든 입력에
  '질문용 Instruct 접두'를 적용한다. 반면 DB 벡터는 접두사 없이 임베딩된 문서다.
  → 따라서 이 스크립트의 유사도는 1.0이 아니라 다소 낮게 나오는 것이 정상이며,
    "완벽 일치" 판정 기준(0.999)은 구버전(대칭 임베딩) 기준이다.
  → 정확한 검증이 필요하면 서버에 instruct 미적용 옵션을 추가한 뒤 사용할 것.
